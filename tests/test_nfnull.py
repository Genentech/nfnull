...
class TestNFNull(unittest.TestCase):
    ...
    def test_nsf_model_with_context(self):
        ...
        p_val_center = model.p_value(
            1.0, greater_than=True, n=10000, context=test_context
        )
        self.assertGreater(p_val_center, 0.3)
        self.assertLess(p_val_center, 0.7)

        # P(X > 1.5) should be much smaller (in the tail)
        p_val_tail = model.p_value(
            1.5, greater_than=True, n=10000, context=test_context
        )
    ...
    def test_tnsf_model_with_context(self):
        ...
        x = (
            context[:, 0]
            + 0.5 * context[:, 1]
            + np.random.standard_t(df=3, size=n_samples)
        )
        ...
        p_val_tail = model.p_value(
            3.0, greater_than=True, n=10000, context=test_context
        )
        self.assertLess(p_val_tail, 0.1)
    ...
    def test_multivariate_tnsf_model(self):
        ...
        t_data_2d = np.column_stack(
            [
                ...
                np.random.standard_t(df, n_samples) * 0.5
                + 0.3 * np.random.standard_t(df, n_samples),
            ]
        )
    ...
    def test_batched_context_operations(self):
        ...
        p_values_scalar = model.p_value(
            scalar_threshold, greater_than=True, n=10000, context=batched_contexts
        )
        ...
        self.assertGreater(
            p_values_scalar[0], 0.01, "Group 0: P(X > 0) should be > 0.01"
        )
        ...
        p_values_array = model.p_value(
            array_thresholds, greater_than=True, n=10000, context=batched_contexts
        )
        ...
        for i, single_context in enumerate(batched_contexts):
            ...
            single_samples = model.sample(
                n=n_samples_test, context=single_context_reshaped
            )
            ...
            single_p_val = model.p_value(
                scalar_threshold,
                greater_than=True,
                n=10000,
                context=single_context_reshaped,
            )
        ...
        with self.assertRaises(ValueError):
            ...
            model.p_value(
                [0.0, 1.0], context=batched_contexts
            )  # 2 thresholds, 3 groups
    ...
    def test_batched_context_multivariate(self):
        ...
        for i, ctx in enumerate(contexts):
            ...
            group_x = np.column_stack(
                [
                    ctx[0]
                    + np.random.normal(0, 0.3, n_samples),  # x1 affected by ctx[0]
                    ctx[1]
                    + np.random.normal(0, 0.3, n_samples),  # x2 affected by ctx[1]
                ]
            )
            all_x.append(group_x)
            all_contexts.extend([ctx] * n_samples)
        ...
        p_values = model.p_value(
            threshold_point, greater_than=True, n=10000, context=batched_contexts
        )
        ...
        p_values_array = model.p_value(
            thresholds_array, greater_than=True, n=10000, context=batched_contexts
        )
